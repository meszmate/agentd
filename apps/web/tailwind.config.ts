  ...
  theme: {
    ...
    extend: {
      ...
      keyframes: {
        ...
        // Vertical drop for `<TransitioningText>`. New line falls in
        // from above and lands in place; old line continues falling
        // down out of view, so the motion reads as a stack rolling
        // top→bottom. translateY uses 100% (one own-line-height) so
        // both lines clear the row exactly. Parent has
        // `overflow-hidden` to clip the off-row halves.
        "label-in": {
      ...
      animation: {
        ...
        "label-out": "label-out 0.32s cubic-bezier(0.55, 0.06, 0.68, 0.19) both",
